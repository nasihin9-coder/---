...
def main():
    ...
    dh = st.sidebar.number_input("Dispersion (Dh)", value=8e-5, format="%.1e")
    q = st.sidebar.number_input("Flux (q)", value=2e-6, format="%.1e")
    ...
    if uploaded_file:
        ...
        if st.button("🚀 开始热-盐耦合解算 (自动优化)", type="primary"):
            ...
            # --- 核心修复：防乱码绘图配置 ---
            # 强制使用 DejaVu Sans，这是 Linux/Streamlit Cloud 唯一 100% 拥有的字体
            plt.rcParams['font.sans-serif'] = ['DejaVu Sans', 'Arial', 'sans-serif']
            plt.rcParams['axes.unicode_minus'] = False # 解决负号显示
            
            fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 5))
            
            # 左图：温度 (使用中英双语标签，英文兜底防止方框)
            ax1.plot(t_obs, z_obs, 'r-o', markersize=4, label='Measured Temp')
            ax1.set_xlabel("Temperature / 温度 (degC)")
            ...
            # 右图：盐度拟合
            ax2.scatter(c_obs, z_obs, color='gray', alpha=0.6, label='Observed Data')
            ax2.plot(c_sim, z_sim, 'g-', linewidth=3, label=f'Model (R2={r2_val:.4f})')
