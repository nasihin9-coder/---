...
st.session_state['dh'] = st.sidebar.number_input(
    "弥散系数 (Dh)", 
    value=st.session_state['dh'], 
    format="%.1e"
)
st.session_state['q'] = st.sidebar.number_input(
    "垂直通量 (q)", 
...
if uploaded_file is not None:
    ...
    # 提供测试数据下载按钮
    with open("dense_experimental_data.csv", "rb") as file:
        btn = st.download_button(
            label="📥 下载测试数据",
            data=file,
            file_name="dense_experimental_data.csv",
            mime="text/csv"
        )
